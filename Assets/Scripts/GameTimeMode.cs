...
public class GameTimeMode : MonoBehaviour
{
    ...
    private int CountDown=59;           //倒计时时长
    ...
    void Update()
    {
        if (IfStart == true)
        {
            ...
            if (Count >= 1.0f)
            {
                Count = 0;
                CountDown--;
                CountText.text = "00:" + CountDown.ToString("d2"); ;
            }
            if(CountDown<=0)
            {
                IfStart = false;
    ...
    {
        GrobalClass.GrobalClassInit();                                  //代替原版Start的工作开始游戏
    ...
    public void OkClick()
    {
        ...
        if (Answer.text == NeedAnswerWord.text)
        {
            ...
            Invoke("GetNewWord", 0.3f);                        //两秒后执行该函数
    ...
    public void GiveupClick()                                   //按钮放弃被点击
    {
        ...
        Msg.text = "弃笔,罚五分";
        GrobalClass.IScore -= 5;
        if (GrobalClass.IScore < 0)
        {
    ...
    public void RealFail()
    {
        ...
        {
            child.GetComponent<Animator>().CrossFade("Presson", 0f); //播放逐渐缩小动画
        }
        OkButtom.SetActive(false);                      //禁用提交答案按钮
        GiveupBottom.SetActive(false);
        Msg.text = "结束";
    ...
    IEnumerator ContinFail()
    {
        ...
        ZJbar.GetComponent<Animator>().CrossFade("左横盖住", 0f);       //播放动画
        FailText.GetComponent<Animator>().CrossFade("下落回弹", 0f);
